 ...
 * 公共类判断不为空
 * */
export const isNotNullOrEmpty = (val) => {
  if (val == "0") {
    return true;
  } else if (val != "" && val != "null" && val != null && val != undefined && val != "undefined") {
    return true;
  } else {
    return false;
  }
...
export const preciseNum = (number, n)=> {
  n = n ? parseInt(n) : 0
  if (n <= 0) {
  ...
  Number.prototype.fixedprecise = function(digits) {
    digits = Math.floor(digits)
    if (isNaN(digits) || digits === 0) {
 ...
 * @returns price
 */
export const iegAmount = (val, type, length) => {
  // priceDigits  单价
  // priceTotalDigits  总价
  // quantityDigits  数量
  // let settingData = store.state.settingData;
  ...
  if (regex.test(val)) {
    if (type == 'per') {
      let price = preciseNum(Number(val), length)
      let priceRe = Number(price).toString()

      let [integer, decimal] = String.prototype.split.call(priceRe, '.')
      integer = integer.replace(/\d(?=(\d{3})+$)/g, '$&,') // 正则先行断言
      return `${integer}${decimal ? '.' + decimal : ''}`

      // return priceRe.replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
      // return priceRe.replace(/(\d{3})(?=\d)/g, '$1,');

    } else if (type == 'total') {
      let totalPrice = preciseNum(Number(val), length)
      let totalPriceRe = Number(totalPrice).toString()

      let [integer, decimal] = String.prototype.split.call(totalPriceRe, '.')
      integer = integer.replace(/\d(?=(\d{3})+$)/g, '$&,') // 正则先行断言
      return `${integer}${decimal ? '.' + decimal : ''}`
      /// return totalPriceRe.replace(/(\d)(?=(\d{3})+\.)/g, '$1,');

    } else if (type == 'count') {
      let count = preciseNum(Number(val), length)
      let countRe = Number(count).toString()

      let [integer, decimal] = String.prototype.split.call(countRe, '.')
      integer = integer.replace(/\d(?=(\d{3})+$)/g, '$&,') // 正则先行断言
      return `${integer}${decimal ? '.' + decimal : ''}`
      /// return countRe.replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
    }
  } else {
    return '0'
  }
