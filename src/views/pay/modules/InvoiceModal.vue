<template>
	<a-drawer title="发票附件" :width="width" placement="right" :closable="false" :headerStyle="{ textAlign: 'center'}"
...
<!--    </div>-->
    ...
    <a-table
      ...
      <template slot='attachment' slot-scope='text,record'>
        <j-upload isMultiple  v-model="record.attachment" disabled></j-upload>
      </template>
    </a-table>
...
<script>
  ...
  export default {
    ...
    props:['contractId','ptype','applyId'],
		components: {
      ...
      loadData(arg) {
        ...
        let url = "/srm/purchasePayInovice/fetchInvoiceList";
