  ...
  <a-card :bordered="false">
    ...
    <div>
      ...
      <a-table ref="table" size="middle" rowKey="id" :scroll="{x:true,y:500}"
        ...
        <template slot='contractNumber' slot-scope="text, record, index">
          <a @click="handleApprove(record)" v-if='record.contractStatus == "4"'>{{text}}</a>
        </template>

				<span slot="action" slot-scope="text, record">
          <a @click="handleApprove(record)">查看详情</a>
				</span>
    ...
    <contract-to-send-modal ref='sendModal'></contract-to-send-modal>
  </a-card>
</template>
...
import ContractToSendModal from '@views/contract/modules/ContractToSendModal'
import { iegAmount } from '@/utils/util'

export default {
  name: "ContractBaseList",
  mixins: [JeecgListMixin, billListMixin, billModalMixin,ContractToSendModal],
  components: {
    ContractToSendModal,
