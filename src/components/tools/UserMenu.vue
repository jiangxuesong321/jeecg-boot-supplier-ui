<template>
  <div class="user-wrapper" :class="theme">
    ...
    <component :is="searchMenuComp" class="borders" :visible="searchMenuVisible" title="搜索菜单" :footer="null" @cancel="searchMenuVisible=false">
      <a-icon type="search"></a-icon>
	  <a-select
        class="search-input"
        showSearch
...
<!--			</a-menu-item>-->
			...
			<a-menu-item key="5" @click="updateAvatar">
			  <a-icon type="cluster"/>
			  <span>头像上传</span>
			</a-menu-item>
    ...
    <a-drawer
      ...
      <div class="drawer-bootom-button" v-show="!disableSubmit">
        ...
        <a-button @click="handleSubmit" type="primary" style='margin-left: 10px'>提交</a-button>
      </div>
    </a-drawer>
	</div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleSubmit(){
        let url = "/sys/user/updateAvatar";
        putAction(url,{id:this.userInfo().id,avatar:this.model.avatar}).then(res => {
          if(res.success){
            this.$message.success("提交成功");
            this.handleCancel();

            let userInfo = this.userInfo();
            userInfo.avatar = this.model.avatar
