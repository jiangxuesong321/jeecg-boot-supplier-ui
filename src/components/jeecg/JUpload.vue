  ...
  methods:{
    ...
    beforeUpload(file){
      ...
      if(this.fileType===FILE_TYPE_IMG){
        ...
        if(fileType.indexOf('zip') >= 0 || fileType.indexOf('RAR') >= 0 || fileType.indexOf('rar') >= 0 ){
          this.$message.warning('不支持上传压缩文件');
          this.uploadGoOn=false
          return false;
        }
      }
...
/*       this.fileList = fileList
    ...
    viewWord(httpUrl){
      ...
      /*
        window.open(urls); */
      // window.open("https://view.xdocin.com/view?src=" + encodeURIComponent(httpUrl));
      let url1 = httpUrl.split("8443")
      let end = "https://ppms.zhlxsemicon.com:8443" + url1[1];
      let toFile = "https://view.xdocin.com/view?src=" + encodeURIComponent(end);
      window.open(toFile);
    },
    handleCancel(){
